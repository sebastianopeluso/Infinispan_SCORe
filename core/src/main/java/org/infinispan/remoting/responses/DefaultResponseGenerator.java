...
public class DefaultResponseGenerator implements ResponseGenerator {
    public Response getResponse(CacheRpcCommand command, Object returnValue) {
        ...
        if (requiresResponse(command.getCommandId(),
                command.getConfiguration().getIsolationLevel() == IsolationLevel.SERIALIZABLE)) { //for prepare command
    ...
    private boolean requiresResponse(byte commandId, boolean serializability) {
        return commandId == ClusteredGetCommand.COMMAND_ID || commandId == GetInDoubtTransactionsCommand.COMMAND_ID
                ...
                || (commandId == PrepareCommand.COMMAND_ID && serializability);
    }
}
