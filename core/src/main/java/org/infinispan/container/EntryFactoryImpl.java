...
public class EntryFactoryImpl implements EntryFactory {
    ...
    public void injectDependencies(DataContainer dataContainer, LockManager lockManager, Configuration configuration, CacheNotifier notifier) {
        ...
        this.isTotalOrder = configuration.isTotalOrderReplication();
    }
        ...
        try {
            ...
            if (cacheEntry != null) // exists in context!  Just acquire lock if needed, and wrap.
            {
                ...
                if (alreadyLocked || ctx.hasFlag(Flag.SKIP_LOCKING) || acquireLock(ctx, key)) {
                    ...
                    mvccEntry.copyForUpdate(container, writeSkewCheck, ctx.isOriginLocal());
                ...
                if (cacheEntry != null) {
                    ...
                    if (needToCopy) mvccEntry.copyForUpdate(container, writeSkewCheck, false);
                    ...
                    ctx.putLookedUpEntry(key, mvccEntry);
                    mvccEntry.copyForUpdate(container, writeSkewCheck, false);
    ...
    public final boolean acquireLock(InvocationContext ctx, Object key) throws InterruptedException, TimeoutException {
        ...
        if(isTotalOrder && ctx.isInTxScope() && !ctx.isOriginLocal()) {
            return acquireLockInTotalOrder(ctx, key);
        }
    ...
    private boolean acquireLockInTotalOrder(InvocationContext ctx, Object key) {
        return true;
    }
}
