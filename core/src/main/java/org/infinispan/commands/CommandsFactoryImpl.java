...
public class CommandsFactoryImpl implements CommandsFactory {
    ...
    public PrepareCommand buildPrepareCommand(GlobalTransaction gtx, List<WriteCommand> modifications,
                                              Object[] readSet, SnapshotId snapshotId,
                                              boolean onePhaseCommit) {
        PrepareCommand command = new PrepareCommand(gtx, onePhaseCommit, readSet, snapshotId, modifications);
        command.setCacheName(cacheName);
        return command;
    }
    ...
    public CommitCommand buildCommitCommand(GlobalTransaction gtx, SnapshotId commitSnapshotId) {
        CommitCommand commitCommand = new CommitCommand(gtx, commitSnapshotId);
        commitCommand.setCacheName(cacheName);
        return commitCommand;
    }
    ...
    @Override
    public AcquireValidationLocksCommand buildAcquireValidationLocksCommand(GlobalTransaction gtx, Set<Object> readSet, Set<Object> writeSet, SnapshotId snapshotId) {
        return new AcquireValidationLocksCommand(gtx, readSet,writeSet, snapshotId);
    }
